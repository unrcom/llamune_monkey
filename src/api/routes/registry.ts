...
router.post('/register', (req, res) => {
  const { instance_id, url, description } = req.body;
  if (!instance_id || !url) {
    res.status(400).json({ error: 'instance_id と url は必須です' });
    return;
  }
  const instance = registry.register({ instance_id, url, description });
...
router.patch('/:instance_id', (req, res) => {
  const { instance_id } = req.params;
  const { model_status, current_model, queue_size, active_request } = req.body;
